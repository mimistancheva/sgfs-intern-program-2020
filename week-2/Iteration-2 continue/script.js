...
//adding events 
var event1  = new Event("Barbie world night", true, 5);
var event2  = new Event("White party", false, 10);
var event3  = new Event("Dj party with Nascosa", false, 10);
var event4  = new Event("Disney party with friends", true);
...
var client1 = new Client("Maria", "Stancheva", true, 19);
var client2 = new Client("Ivan", "Todorov", false, 18);
var client3 = new Client("Nikolai", "Cvetanov",true, 20);
var client4 = new Client("Veronika", "Lubenova", false, 11);
...
function addClientToEvent(client, event){
    ...
    if(closeSystem == true){
        ...
        if (client.age < MIN_AGE && event.flag == true) {   
            console.log(`The client ${client.firstName} ${client.lastName} was added to ${event.name}`);    
            //console.log("This event is for +18");
        }else
        if (client.age < MIN_AGE && event.flag == false) {
            console.log("This event is for +18");
        }else
        if (client.age >= MIN_AGE){
            console.log(`The client ${client.firstName} ${client.lastName} was added to ${event.name}`);
        }
