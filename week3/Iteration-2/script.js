...
//number of events that client attends
var numberOfeventsAttended = 0;
...
//adding events 
var event1  = new Event("White party", true);
var event2  = new Event("Barbie world night", false, 10);
var event3  = new Event("Disney party with friends", false, 10);
var event4  = new Event("Dj party with Nascosa", true, 5);
...
deleteEvent(4);
console.log("Events after deleting an event.");
listEvents();
...
function mostClientsToEvent(){
   ...
   /* console.log(`Events with the most clients are:`);
    for (let ev of eventList) {

        if(ev.clientsList.length == clientNumber + 2){
        console.log(` ${ev.name}`);
        
    console.log(`Events with 1 client are:`);
    for (let ev of eventList) {
    ...
    console.log(`There are no clients in the event with name:`);
    for (let ev of eventList) {
...
createEvent("Retro party mix", false);
addClientToEvent(client4,event1);
...
//VIP client
function VIPclient(){
    return numberOfeventsAttended % 5 == 0;
}
